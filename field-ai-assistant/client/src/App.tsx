...
export default function App() {
  const sharedWorkspace = !['localhost', '127.0.0.1', '[::1]'].includes(window.location.hostname);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-foot"><span className="local-indicator" />{sharedWorkspace ? '共有ワークスペース' : 'ローカルワークスペース'}<small>在庫の変更は、あなたの確認から。</small></div>
    </aside>
    <div className="workspace">
      ...
      <main>
        ...
        <footer className="workspace-footer"><span>カメラでは一般カテゴリを判定し、登録商品として扱います。</span><span>{sharedWorkspace ? '保存先：Googleスプレッドシート' : '保存先：この端末'}</span></footer>
      </main>
    </div>
